...
class TestPerceptron(unittest.TestCase):
    def setUp(self):
        ...
        self.train_points = np.array([[0, 0, 1], [1, 0, 1], [0, 1, 1], [1, 2, 2], [2, 1, 2], [2, 2, 2]])
        self.formatted_training_points = np.array([[0, 0, 1], [1, 0, 1], [0, 1, 1]]), \
                                         np.array([[1, 2, 1], [2, 1, 1], [2, 2, 1]])
        self.iterations = 100
        self.final_weights = np.array([-0.2, -0.6, 0.7])

    def create_perceptron(self):
        return Perceptron(self.weights, self.learning_rate, self.train_points, self.iterations, lambda x: x)
    ...
    def test_constructor_learning_rate_set(self):
        ...
        self.assertEqual(perceptron._Perceptron__learning_rate, self.learning_rate)
    ...
    def test_constructor_train_set(self):
        ...
        self.assertTrue((perceptron_train[0] == self.formatted_training_points[0]).all() and
                        (perceptron_train[1] == self.formatted_training_points[1]).all())

    def test_format_training_points(self):
        # Check if the incoming training points are formatted correctly for classification
        perceptron = self.create_perceptron()
        fomatted_points = perceptron._Perceptron__format_training_points(self.train_points)
        self.assertTrue((fomatted_points[0] == self.formatted_training_points[0]).all() and
    ...
    def test_final_weights(self):
        ...
        self.assertTrue(np.isclose(perceptron._Perceptron__weights, self.final_weights).all())
